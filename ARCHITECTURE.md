...
### ADR-005: CLI resource verbs — client, YAML↔proto codec, `apply` semantics

**Context.** Phase 0 needs the kubectl-style resource verbs (`apply`, `get`, …) so the running control plane is usable from the terminal and the end-to-end test can drive it. This requires a gRPC client, a YAML↔protobuf codec, and a decision on `apply` semantics. ADR-001 already established that `apiVersion`/`kind` live only at the YAML boundary, not in the proto.

**Decision.**

- **YAML↔proto via protojson.** YAML documents are parsed (`gopkg.in/yaml.v3`) into string-keyed maps, the `apiVersion`/`kind` envelope is stripped, and the remaining `{metadata, spec, status}` body is re-marshaled to JSON and fed to `protojson.Unmarshal`. protojson means YAML field names follow the proto contract (camelCase `displayName`, enum names like `APPROVAL_POLICY_HUMAN`) automatically, and unknown fields are rejected — a typo in a spec field is a hard error, not a silent drop. Output runs the reverse and stamps the envelope back on, so `get -o yaml` round-trips cleanly through `apply`. (Encoding decodes the protojson body into generic `map[string]any`, not `json.RawMessage`, so `yaml.Marshal` renders real maps/scalars.)
- **TypeMeta validated at the boundary.** `apiVersion` must equal `kestrai.dev/v1alpha1` and `kind` must be a known type; both are consumed before the proto sees the body. This realizes ADR-001's "CLI stamps TypeMeta on marshal, consumes it on unmarshal" without a TypeMeta message in the wire types.
- **`apply` is create-or-update (kubectl semantics).** For each resource the verb does a Get: on `NotFound` it Creates; otherwise it copies the stored `resourceVersion` onto the object and Updates, so the optimistic-concurrency check passes. Re-applying the same file is idempotent (reports `created` then `configured`).
- **Multi-document files.** A single `-f` file may hold many resources separated by `---`; each is applied in order. `-f -` reads stdin.
- **`get` defaults to a table, `-o yaml|json` for the full object.** Tables render with `text/tabwriter` (NAME/DISPLAY/READY/… columns and a short `AGE`). A single object needs its scope (`-p/--project` for a Workflow); list omits it. Matches kubectl's "table by default, structured on request" and the DX bar's "JSON only behind a flag."
- **Client config: flags + `KESTRAI_*` env, Viper deferred.** `--address`/`--token`/`--output` are persistent flags with `KESTRAI_ADDRESS`/`KESTRAI_TOKEN` env defaults, shared via a `clientConfig` passed to each verb constructor. The client dials plaintext for local-dev and attaches a bearer token via `PerRPCCredentials` when one is set (static-token servers). TLS and a kubeconfig-style context file arrive with the auth-client work; Viper lands when layered config earns its keep.

**Consequences.** Adding a verb for a new resource is mechanical: register it in `newMessageForKind`/`normalizeKind` and add a row formatter. The codec is the single place YAML meets proto, so the wire contract cannot drift between input and output. `apply`'s read-modify-write has a benign lost-update window (two concurrent applies can make one retry); acceptable for a single-user local CLI. Covered by a codec unit test (multi-doc decode, error cases, encode round-trip) and an `apply`→`get` integration test over a real gRPC connection, plus a manual `kestrai up` smoke run.
